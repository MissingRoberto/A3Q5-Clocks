...
public class Main {
	public static void main(String[] args) {
		...
		Timeclient p_1 = new Timeclient(new Clock("1", incrementTime, 0.05),
				p1_addr, "P2");
		Timeserver p_2 = new Timeserver(new Clock("2", incrementTime, 0.00),
				p2_addr);
		Timeclient p_3 = new Timeclient(new Clock("3", incrementTime, 0.17),
				p3_addr, "P3");
		Timeclient p_4 = new Timeclient(new Clock("4", incrementTime, -0.11),
				p4_addr, "P4");

		EventQueue eventQueue = EventQueue.getInstance();
		eventQueue.addChannel(p1_addr, p2_addr, 110);
		eventQueue.addChannel(p2_addr, p1_addr, 135);
		eventQueue.addChannel(p2_addr, p3_addr, 140);
		eventQueue.addChannel(p3_addr, p2_addr, 190);
		eventQueue.addChannel(p2_addr, p4_addr, 245);
		eventQueue.addChannel(p4_addr, p2_addr, 145);
